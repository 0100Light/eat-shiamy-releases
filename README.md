...
跟系統內建的微軟注音一起用時，照下面兩張圖調過會順手一些。路徑是
**設定 → 時間與語言 → 語言與地區 → 中文(繁體，台灣) → 選項 → 微軟注音 → 選項**。

<img src="img/ime-mstw-general.png" width="500" alt="微軟注音「一般」的輸入設定">

其中**相容性的「使用舊版的微軟注音」建議開啟**：不開的話，本程式送出的標點
會被新版微軟注音收進它自己的組字狀態，畫面上會看到標點底下多一條虛線、
要再按一次 Enter 才會真的出現。

<img src="img/ime-mstw-compat.png" width="500" alt="微軟注音的相容性設定：使用舊版的微軟注音">
